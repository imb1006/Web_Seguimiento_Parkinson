...
void printMedidas()
{
  // Funcion que muestra los valores de salida de los sensores inerciales en el Sistema Internacional. Pantalla LCD
  ...
  // Enviar los mismos datos a través de Bluetooth
  btSerial.print(F("a[x y z](m/s2):\t"));
  btSerial.print(ax * accEscala); btSerial.print("\t");
  btSerial.print(ay * accEscala); btSerial.print("\t");
  btSerial.println(az * accEscala); 
  btSerial.print(F("g[x y z](deg/s):\t"));
  btSerial.print(gx * gyroEscala); btSerial.print("\t");
  btSerial.print(gy * gyroEscala); btSerial.print("\t");
  btSerial.println(gz * gyroEscala);
...
void setup()
{
  ...
  btSerial.println("Presionar START");
